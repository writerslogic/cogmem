...
def _engine(script: str, *args: str, parse_json: bool = False, timeout: int = 30):
    out = subprocess.run(
        [PY, str(ENGINE / script), *args], capture_output=True, text=True, timeout=timeout
    )
    if parse_json:
...
DISPATCH = {
    ...
    "tree_head": _tool_tree_head,
    "progress": _tool_progress,
    "review_pending": _tool_review_pending,
}
    ...
    },
    {
        ...
        "outputSchema": {
            ...
            "properties": {
                ...
                "logEntries": {"type": "integer"},
                "logIntegrity": {"type": "string"},
...
def _list_resources() -> list:
    ...
    if pdir.exists():
        for f in sorted(pdir.glob("*.md")):
            res.append(
                {
                    "uri": f"cogmem://project/{f.stem}",
                    "name": f"Project: {f.stem}",
                    "mimeType": "text/markdown",
...
def _handle(req):
    ...
    if method == "tools/call":
        ...
        try:
            ...
            return _result(
                rid, {"content": [{"type": "text", "text": f"error: {exc}"}], "isError": True}
            )
